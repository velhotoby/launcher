...
function validateFilename(filename) {
  return typeof filename === 'string' && filename.endsWith('.jar')
  ...
  for (const rawUrl of mod.urls) {
    try {
      ...
      const response = await fetch(rawUrl, {
        redirect: 'follow', headers: { 'User-Agent': 'CobblemonLegacyLauncher/3.4.25' }
      });
...
async function loadCatalog(configuration, notify, gamePath) {
  ...
  const response = await fetch(remoteUrl, { headers: {
    'User-Agent': 'CobblemonLegacyLauncher/3.4.25',
...
module.exports = { syncTrustedMods, validateCatalog, validateFilename, downloadMod, loadCatalog };
